...
def main():
    ...
    try:
        if args.config:
            ...
            proxmox = dict(configFile._sections['proxmox'])
            cloudinit = dict(configFile._sections['cloudinit'])
    ...
    try:
        ...
        disk_size = int(proxmox['disk']) * 1024 ** 2
        ...
        api.move_base_disk(node=proxmox['node'],storage=proxmox['storage'], vmid=proxmox['vmid'], disk="virtio0")
        api.move_base_disk(node=proxmox['node'],storage=proxmox['storage'], vmid=proxmox['vmid'], disk="virtio1")
